...
import { User as UserWithExtras } from "@/lib/definitions";
    ...
    <Tabs defaultValue="posts" className="pt-14 md:pt-32 pb-16">
      <TabsList className="p-px bg-zinc-300 dark:bg-neutral-800 h-px w-full gap-x-10">
        {profileTabs
          ...
          .map((tab) => {
            const profilePage = `/dashboard/users/${profile.username}`;
            const isActive =
              tab.href === ""
            ...
            return (
              <TabsTrigger
                ...
                asChild
              >
                <Link href={`/dashboard/users/${profile.username}/${tab.href}`}>
                  <Separator
                    className={cn(
                      "!h-px w-16",
                      isActive
